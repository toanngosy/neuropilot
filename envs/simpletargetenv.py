...
class SimpleTargetEnv3D(gym.Env):
    ...
    def render(self, mode='human', close=False):
        if self.viewer is None:
            ...
            self.ax[0, 0].set_xlim([self.min_x-5, self.max_x+5])
            self.ax[0, 0].set_xlabel('X')
            self.ax[0, 0].set_ylim([self.min_y-5, self.max_y+5])
            self.ax[0, 0].set_ylabel('Y')
            self.agentplotxy, = self.ax[0, 0].plot([], [], marker='o', color='blue', markersize=6, antialiased=False)
            self.targetplotxy, = self.ax[0, 0].plot([], [],  marker='o', color='red', markersize=6, antialiased=False)

            self.ax[0, 1].set_xlim([self.min_x-5, self.max_x+5])
            self.ax[0, 1].set_xlabel('X')
            self.ax[0, 1].set_ylim([self.min_z-5, self.max_z+5])
            self.ax[0, 1].set_ylabel('Z')
            self.agentplotxz, = self.ax[0, 1].plot([], [], marker='o', color='blue', markersize=6, antialiased=False)
            self.targetplotxz, = self.ax[0, 1].plot([], [],  marker='o', color='red', markersize=6, antialiased=False)

            self.ax[0, 2].set_xlim([self.min_y-5, self.max_y+5])
            self.ax[0, 2].set_xlabel('Y')
            self.ax[0, 2].set_ylim([self.min_z-5, self.max_z+5])
            self.ax[0, 2].set_ylabel('Z')
            self.agentplotyz, = self.ax[0, 2].plot([], [], marker='o', color='blue', markersize=6, antialiased=False)
            self.targetplotyz, = self.ax[0, 2].plot([], [],  marker='o', color='red', markersize=6, antialiased=False)
